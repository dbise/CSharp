...
namespace Lab12
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var divisibleByThree = from n in odds 
                                   where (n % 3) == 0 
                                   select n;
            ...
            //Query1 – Find the values in evens and the values in odds where the evens values are greater than two (2) 
            //times the value of odds values. Write these values to the Console.

            var twotimessize = from l1 in odds
                               from l2 in evens
                               where l2 >= l1 * 2
                               select new
                               {
            ...
            foreach (var item in twotimessize)
            {
                myKey = item.key;
                if(changekey != myKey)
                {
            ...
            //Query2 – Find values in data where the ID is greater than 100 and less than 500 and the Name contains a ‘y’ or Job ends with ‘e’ 
            //or Job contains ‘e’ or Job equals “Student” or Job equals “Programmer”. Write these values to the Console.
            var dataFinder = from d in data
                             where d.ID > 100 && d.ID < 500 && (d.Name.Contains('y') || d.Job.EndsWith('e') || d.Job.Contains('e') || d.Job == "Student" || d.Job == "Programmer")
                             select d;


            foreach (var i in dataFinder)
            {
            ...
            //Query 3 – Find values in productList where the ProductID is greater than 10 and less than 66, order in descending order by 
            //UnitsInStock and write the ProductName and UnitsInStock to the Console.

            var myList = GetProductList();

            var pList = from t in myList
                           where t.ProductID > 10 && t.ProductID < 66
                           orderby t.UnitsInStock descending
                           select t;


            foreach (var i in pList)
            {
                Console.WriteLine(i.ProductName + " - " + i.UnitsInStock +", ");
            ...
            //Query4 – Write a complex query on the productList that shows you know how to use at least ten (10) different LINQ operations 
            //i.e. let, where, orderby, into, from, etc.


            var joinone = from s in myList//1
                             where s.Category == "Dairy Products"//2
                             orderby s.UnitPrice//3
                             descending//4
                             select s;//5

            var jointwo = from w in myList
                           where w.Category == "Meat/Poultry"                   
                           orderby w.UnitPrice
                           ascending //6
                           select w;

            var combine = joinone.Concat(jointwo).ToList(); //7

            var ordercombined = combine.OrderBy(name => name.ProductName.Length).ThenBy(name => name.UnitsInStock);//8

            var atleastten = from q in ordercombined.Distinct()//9
                              let possiblesales = q.UnitsInStock * q.UnitPrice //10
                              where possiblesales >0
                              select q;



            foreach (var i in atleastten)
            {
